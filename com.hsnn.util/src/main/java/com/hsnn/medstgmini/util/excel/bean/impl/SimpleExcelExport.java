 ...
 * excel导出
 *
 * @ClassName: SimpleExcelExport
 * @author zhou.xy
 * @date 2016年4月11日 上午10:03:22
 *
 */
public class SimpleExcelExport implements IExcelExport {
	 ...
	 */
	/*
	 * private Map<String,PropertyDescriptor> fieldInfo; private
	 * Map<String,AnnExcelColumn> fieldAnnInfo; private
	 * ArrayList<AnnExcelColumn> titleOrder;
	 */
	// 处理对象的所有的注解相关的信息
	 ...
	 * @throws Exception
	 */
	public <T> void execute(OutputStream os, List<T> data, Class<?> clazz, boolean isError) throws Exception {
		/**
		 * 1.获取实例的注解信息 2.获取实例字段注解信息 3.准备导出的字段 4.创建工作薄 5.创建工作表 6.输出标题行 7.输出数据
		 */
		...
		// @4
		Workbook workbook = new XSSFWorkbook();// HSSFWorkbook 97-2003 版本的 ,
		...
		if(isError) {
			for (int i = 0, size = titleOrder.size(); i < size; i++) {// 只会调用一次
																		// .size()
																		// 方法，据说性能会好点
			...
			for (int i = 1, size = titleOrder.size(); i < size; i++) {// 只会调用一次
																		// .size()
																		// 方法，据说性能会好点
			Cell titleCell = titleRow.createCell(i);
			titleCell.setCellValue(titleOrder.get(i).title());
		...
		if(data != null && data.size() > 0) {
			for (T t : data) {
				...
				if(isError) {
					...
					for (int j = 1, size = titleOrder.size(); j < size; j++) {// 创建数据列
						Cell cell = row.createCell(j);
	...
	private void cellSetValue(Cell cell, Object t, Workbook workbook,
			String title) throws IllegalAccessException,
			IllegalArgumentException, InvocationTargetException,
			ExcelEnumTypeException {
		// 获取对象的值
		...
		} else if (valueClazz == Integer.class || valueClazz == Long.class
				|| valueClazz == Double.class || valueClazz == BigDecimal.class
	...
	private String enumProcess(Object source, Class enumtype)
		...
		if (ClassUtil.isImplement(enumtype, IExcelCommonEnum.class)) {
			...
			throw new ExcelEnumTypeException(enumtype.getName() + " 没有找到枚举值："
					+ source);
		}
