...
@Service
public class SysUserYimiaoManagerImpl implements ISysUserYimiao {
	private static final Logger log = Logger.getLogger(SysUserYimiaoManagerImpl.class);
	...
	public void sendEmail(SysUser user, String address,Pagination page) throws Exception {
        ...
        StringBuilder url =new StringBuilder();
				...
				EmailUtil.sendTextMail(prop, "找回贵州省第二类疫苗集中采购系统密码",body(url.toString(),user.getUserName()));
	...
	public String body(String url,String userName){
		...
		.append("&nbsp;&nbsp;您正在进行贵州省第二类疫苗集中采购系统密码找回，点击以下链接完成验证，重新设置密码。</br>")
		...
		.append("这是云南省第二类疫苗集中采购系统的系统邮件,请勿回复。");
		...
		if(StringUtils.isNotBlank(postId)){
			...
			if(StringUtils.isNotBlank(adds)){
				...
				for(int i=0;i<add.length;i++){
					...
					srr.setAddUserId(updUserId);
					srr.setAddUserName(userName);
					srr.setAddTime(new Date(System.currentTimeMillis()));
					/*srr.setLastUpdateUserId(updUserId);
					srr.setLastUpdateUserName(userName);
					srr.setLastUpdateTime(new Date(System.currentTimeMillis()));*/
	...
	public SysUser getUserByUsername(String username) {
		...
		if (sysUser != null) {
			...
			//判断部门是否为空
			if(StringUtils.isNotEmpty(sysUser.getDepartmentId()+"")) {
			...
			if (sysRoleDao.addSysRole(sysRole)) {
				...
				if(list!=null && list.size()>0){
					for(int i =0;i<list.size();i++){
						...
						sysRoleResource.setAddTime(updTime);
						sysRoleResource.setAddUserId(userId);
						sysRoleResource.setAddUserName(updUser);
						sysRoleResource.setLastUpdateTime(updTime);
