...
<section class="content-header">
	<h1><spring:message code="配送企业采购情况"/></h1>
	<ol class="breadcrumb">
		...
		<li class="active"><a href="#"><spring:message code="配送企业采购情况"/></a></li>
	</ol>
</section>
	...
	<div class=" box box-primary">
		<div class="box-body">
			<form id="form" method="post" >
				...
				<div class="textalign">
					<button id="selectSpec" type="button" onclick="search();" class="btn btn-success btn-sm"><spring:message code="message.button.seachSpacing"/></button>				</div>
...
<script type="text/javascript">
    var obj ={
        ...
        "model": [
            {name:'oper',index:'formName', width:100, align:"center", sortable:false,formatter:function(val,option,rowdata){
                    ...
                    var str =
						"<a href='javascript:void(0)' onclick=\"xgTz('"+rowdata.companyIdPs+"','"+date+"','"+rowdata.companyNamePs+"','"+rowdata.purchaseAmount+"','"+rowdata.purchaseAmount+"','"+rowdata.totalWarehouseAmount+"','2')\"  style='background:url(${ctx}/lib/img/icon/icon.png) no-repeat -40px -80px' class='opIcon' title='采购药品'></a>";
                    str += "<a href='javascript:void(0)' onclick=\"xgTz('"+rowdata.companyIdPs+"','"+date+"','"+rowdata.companyNamePs+"','"+rowdata.purchaseAmount+"','"+rowdata.purchaseAmount+"','"+rowdata.totalWarehouseAmount+"','1')\"  style='background:url(${ctx}/lib/img/icon/icon.png) no-repeat -20px -120px' class='opIcon' title='采购医院'></a>";
                    str += "<a href='javascript:void(0)' onclick=\"xgTz('"+rowdata.companyIdPs+"','"+date+"','"+rowdata.companyNamePs+"','"+rowdata.purchaseAmount+"','"+rowdata.purchaseAmount+"','"+rowdata.totalWarehouseAmount+"','4')\"  style='background:url(${ctx}/lib/img/icon/icon.png) no-repeat -40px -120px' class='opIcon' title='采购生产企业'></a>";
                    return str;
                }},
            ...
            { name: 'companyNamePs', width: 80, align:"center", sortable: false},
    ...
    $(function () {
        ...
        $("#gridlist").jqGrid({
            url: "${ctx}/yimiaoSupervise/getDataByPS.html",
            mtype: "post",
            datatype: "json",
            autowidth: true,
            height: 310,
    ...
    function xgTz(delCompCode,date,delCompName,total,del,recp,type) {
        //type 1医疗机构2药品3地区4生产企业
        window.location.href="${ctx}/yimiaoSupervise/toAnaByCompTz.html?dateType=mon&delCompCode="+delCompCode+"&date="+date+"&delCompName="+delCompName+"&total="+total+"&del="+del+"&recp="+recp+"&type="+type;
    }

    function exportExcel(){
        var postData = $("#form").serializeJSON();
        gridParam(postData);
        $.StandardPost("${ctx}/yimiaoSupervise/exportExcelWithPS.html", postData);
    }
    var objTemp = null;
