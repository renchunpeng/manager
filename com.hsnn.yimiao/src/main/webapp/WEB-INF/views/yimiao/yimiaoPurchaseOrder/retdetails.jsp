...
<body class="skin-blue sidebar-mini fixed skin-blue-light-frame">
	...
	<section class="content">
		<div class=" box box-primary">
			<div class="box-body">
				<form action="${ctx }/yimiaoPurchaseOrder/getretYimiaoOrderdetailData.html" id="searchForm" method="post" >
					<div class="input">
		...
		<div class="btn-control-box">
			...
			<button id="save" type="button" class="btn btn-success btn-sm" onclick="save();">修改退货数量</button>
			<button id="add" type="button" class="btn btn-success btn-sm" onclick="goAdd();">新增疫苗</button>
			<button id="del" type="button" class="btn btn-success btn-sm" onclick="goDelete();">删除疫苗</button>
...
<script type="text/javascript">
        ...
        "model": [
			{ name: 'operate', width:80, align: 'center',sortable: false,hidden:true,
            	formatter:function(val, opts, rowdata){
                	var str =  "<a href=\"javascript:void(0)\" onclick=\"goDelete('"+rowdata.orderdetailRetId+"')\" >删除</a>";
                	return str;
                }
        	},
		        { name: "orderdetailRetId", key:true, sortable: false,hidden:true},
		        { name: "orderId", sortable: false,hidden:true},
                { name: "orderDetailId", sortable: false,hidden:true},
		        { name: "totalReturnCount", sortable: false,
		        	formatter:function(val, opts, rowdata){
	                	var str =  "<input id='retCount"+rowdata.orderdetailRetId+"' value="+val+">";
	                	return str;
	                }
		        },<!-- 退货数量 -->
		        { name: "returnReason", width: 100, align: 'center' ,sortable: false},
		        { name: "totalReturnAmount", sortable: false,
		        	formatter:function(val, opts, rowdata){	
		        		if(val!=null){
	...
	$(function(){
		...
		$("#gridlist").jqGrid({
            url: "${ctx }/yimiaoPurchaseOrder/getretDetails.html?orderId=${orderId}",
            colNames: obj.names,
            colModel: obj.model,
            postData: queryData,
            multiselect: true,
	...
	function goAdd(){
		window.location.href="${ctx}/yimiaoPurchaseOrder/toretOrderList.html?orderId=${orderId}";
	}
	...
	function goDelete(){
		...
		if (rowIds.length <= 0) {
	    	top.$.HN.message.alert("请至少选择一条退货明细！", "消息", "warn");
	        return false;
		}
		
	$.HN.message.confirm('确定要删除选中的退货明细吗？', '', '').on(function (e) {
		if (e) {
				$.ajax({
					url:'${ctx }/yimiaoPurchaseOrder/getretDelete.html',
					type:"post",
					dataType : "json",
	...
	function save(){
		...
		for (var i=0; i<rowIds.length; i++) {
	        ...
	        var retCount = $("#retCount"+orderdetailRetId).val();
	        if (!/^\+?[1-9][0-9]*$/.test(retCount)) {
	        	top.$.HN.message.alert("勾选的第"+(i+1)+"个退货数量必须是正整数！", "消息", "warn");
		        return false;
	        }
		...
		$.HN.message.confirm('确定要保存选中疫苗的数量吗？', '', '').on(function (e) {
			if (e) {
				$.ajax({
					url:'${ctx }/yimiaoPurchaseOrder/updateRetDetailCount.html',
					type:"post",
					dataType : "json",
					data:{"str":JSON.stringify(list)},
               		success: function(result) {
