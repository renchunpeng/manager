...
<body class="skin-blue sidebar-mini fixed skin-blue-light-frame">
	...
	<section class="content">
		<form  id="addForm" role="form" class="form-horizontal" action="addOrder.html" method="post">
			 			...
			 			<table class="form-table form-table-2 form-table-info">
							<tbody>
								<tr class="form-group-sm">
									<th>
			                            <label class="control-label ">采购单名称：</label>
			                        </th>
			                        <td>
								...
								<tr class="form-group-sm">
			                        ...
			                        <td>
			                        	 <textarea class="form-control" id="orderRemarks" name="orderRemarks" rows="3" placeholder="备注"  maxlength="256" style="width:340px;"></textarea>
			                        </td>
								</tr>
							</tbody>
						</table>
...
function nextStep(){
	var orderRemarks = $.trim($('#orderRemarks').val());
	var orderName = $.trim($('#orderName').val());

    if(orderName==""){
    	$.HN.message.alert('请输入采购单名称！', '提示信息', 'warn');
     	 return;
    }
  	if(orderName.length>50){
  		$.HN.message.alert('采购单名称长度不能超过50！', '提示信息', 'warn');
      	 return;
  	}
	if(orderRemarks.length>520){
    ...
    $("#addForm").ajaxSubmit({
        ...
        success: function(result) {
            if(result.success){
                var url = "${ctx}/yimiaoPurchaseOrder/toOrderList.html?orderId="+result.msg;
