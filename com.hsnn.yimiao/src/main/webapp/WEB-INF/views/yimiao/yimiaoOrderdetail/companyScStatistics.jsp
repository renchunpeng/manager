...
<title><spring:message code="生产企业汇总分析"/></title>
<%@include file="/WEB-INF/component/commonCSS.jsp"%>
</head>
<body class="skin-blue sidebar-mini fixed skin-blue-light-frame">
	<section class="content-header">
		<h1><spring:message code="生产企业汇总分析" /></h1>
		<ol class="breadcrumb">
			<li><a href="${ctx}/home.html"><i class="fa fa-home"></i><spring:message code="message.lable.first" /></a></li>
			<li class="active"><a href="#"><spring:message code="生产企业汇总分析" /></a></li>
		</ol>
	</section>
		...
		<div class=" box box-primary width-control">
			...
			<div class="box-body search-box">
				<form id="searchForm" action="" method="post">
					...
					<div class="input">
						 <div class="textalign1">地区<spring:message code="message.yanZheng.maoHao" /></div>
						<div class="boxinput">
...
var obj={
	...
	"model":[
		...
		{name:"operate", width: 330, align: 'center',sortable: false, align:'center',
			formatter:function(val, opts, rowdata){
				var str = "";
				str += "<a href='javascript:void(0);' onclick='toOtherList(\""+rowdata.companyIdSc+"\",\""+rowdata.companyNameSc+"\",\"stdHospital\")' class='opIcon' title='相关医院'>相关医院</a>";
				str += "<a href='javascript:void(0);' onclick='toOtherList(\""+rowdata.companyIdSc+"\",\""+rowdata.companyNameSc+"\",\"stdArea\")' class='opIcon' title='相关地区'>相关地区</a>";
				str += "<a href='javascript:void(0);' onclick='toOtherList(\""+rowdata.companyIdSc+"\",\""+rowdata.companyNameSc+"\",\"yimiaoProcurecatalog\")' class='opIcon' title='相关疫苗'>相关疫苗</a>";
				str += "<a href='javascript:void(0);' onclick='toOtherList(\""+rowdata.companyIdSc+"\",\""+rowdata.companyNameSc+"\",\"stdCompanyPs\")' class='opIcon' title='相关配送企业'>相关配送企业</a>";
				return str;
			}
    	},
    	{name:"companyIdSc", sortable:false, align:'center', hidden:true}
	]
};
...
function initGrid(){
	...
	$("#gridlist").jqGrid({
		url:"${ctx}/yimiaoOrderdetail/getCompanyScStatistics.html",
		mtype:"post",
		datatype:"json",
		autowidth:true,
		shrinkToFit:false,
...
//跳转页面
function toOtherList(companyIdSc,companyNameSc,type){
	$("#companyIdSc").val(companyIdSc);
	$("#companyNameSc").val(companyNameSc);
	...
	if(type == 'yimiaoProcurecatalog'){
		$("#searchForm").attr('action',"${ctx}/yimiaoOrderdetail/toYimiaoProcurecatalogStatistics.html");
	}
	$("#searchForm").submit();
}
...
function queryCount(){
	...
	var queryData = $("#searchForm").serializeJSON();
	$.ajax({
		url:"${ctx}/yimiaoOrderdetail/getYimiaoCompanyScCount.html",
		data:queryData,
		dataType:'json', 
...
$("#exportExcel").click(function(){
	if(queryInit()==false){
		...
		var queryData = $("#searchForm").serializeJSON();
		$.StandardPost("${ctx}/yimiaoOrderdetail/exportDataCompanySc.html",queryData);
	}
});
</script>
</html>