...
<title><spring:message code="疫苗汇总分析"/></title>
<%@include file="/WEB-INF/component/commonCSS.jsp"%>
</head>
<body class="skin-blue sidebar-mini fixed skin-blue-light-frame">
	<section class="content-header">
		<h1><spring:message code="疫苗汇总分析" /></h1>
		<ol class="breadcrumb">
			<li><a href="${ctx}/home.html"><i class="fa fa-home"></i><spring:message code="message.lable.first" /></a></li>
			<li class="active"><a href="#"><spring:message code="疫苗汇总分析" /></a></li>
		</ol>
	</section>
				...
				<form id="searchForm" action="" method="post">
					...
					<div class="input">
	                    ...
	                    <div class="boxinput">
	                        ...
	                        <input type="hidden" id="procurecatalogId" name="procurecatalogId" value="${queryMap.productName }">
	                    </div>
...
var obj={
	...
	"model":[
		{ name: "procurecatalogId", width: 80, align: 'center' ,sortable: false},
		{ name: "productName", width: 150, align: 'center' ,sortable: false},
		{ name: "outlook", width: 150, align: 'center' ,sortable: false},
		{ name: "unit", width: 100, align: 'center' ,sortable: false},
		...
		{name:"operate", width: 350, align: 'center',sortable: false, align:'center',
			formatter:function(val, opts, rowdata){
				var str = "";
				str += "<a href='javascript:void(0);' onclick='toOtherList(\""+rowdata.procurecatalogId+"\",\""+rowdata.productName+"\",\"stdHospital\")' class='opIcon' title='相关医院'>相关医院</a>";
				str += "<a href='javascript:void(0);' onclick='toOtherList(\""+rowdata.procurecatalogId+"\",\""+rowdata.productName+"\",\"stdArea\")' class='opIcon' title='相关地区'>相关地区</a>";
				str += "<a href='javascript:void(0);' onclick='toOtherList(\""+rowdata.procurecatalogId+"\",\""+rowdata.productName+"\",\"stdCompanyPs\")' class='opIcon' title='相关配送企业'>相关配送企业</a>";
				str += "<a href='javascript:void(0);' onclick='toOtherList(\""+rowdata.procurecatalogId+"\",\""+rowdata.productName+"\",\"stdCompanySc\")' class='opIcon' title='相关生产企业'>相关生产企业</a>";
				return str;
			}
    	}
	]
...
function initGrid(){
	...
	$("#gridlist").jqGrid({
		url:"${ctx}/yimiaoOrderdetail/getYimiaoProcurecatalogStatistics.html",
		mtype:"post",
		datatype:"json",
		autowidth:true,
		shrinkToFit:false,
...
//跳转页面
function toOtherList(procurecatalogId,productName,type){
	$("#productName").val(productName);
	$("#procurecatalogId").val(procurecatalogId);
...
function queryCount(){
	...
	var queryData = $("#searchForm").serializeJSON();
	$.ajax({
		url:"${ctx}/yimiaoOrderdetail/getYimiaoProcurecatalogCount.html",
		data:queryData,
		dataType:'json', 
...
$("#exportExcel").click(function(){
	if(queryInit()==false){
		...
		var queryData = $("#searchForm").serializeJSON();
		$.StandardPost("${ctx}/yimiaoOrderdetail/exportDataYimiaoProcurecatalog.html",queryData);
	}
});
</script>
</html>