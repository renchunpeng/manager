...
<body class="skin-blue sidebar-mini fixed skin-blue-light-frame">
	...
	<section class="content">
		<div class=" box box-primary width-control">
			...
			<div class="box-body search-box">
				<form id="searchForm" action="" method="post">
					...
					<input type="hidden" id="goods" name="goods" value="1" />
				</form>
			</div>
		</div>
		...
		<div id="countDiv" class="btn-control-box width-control" style="margin-top:2px; display:none;">
			统计：采购金额（未减去撤单金额与配送金额）为<span id="purchaseAmountSpan" style="color:red;"></span>元，入库金额为<span id="totalWarehouseAmountSpan" style="color:red;"></span>元
		</div>
...
var obj={
	"names":['通用名','剂型','规格','包装','单位','生产企业','采购价格(元)','采购金额(元)','撤废金额(元)','退货金额(元)','退货率','入库金额(元)','入库率','查看','其他','医院id'],
	"model":[
        {name:"productName", sortable:false, align:'center'},
        {name:"medicinemodel", sortable:false, align:'center'},
        {name:"outlook", sortable:false, align:'center'},
        {name:"materialName", sortable:false, align:'center'},
        {name:"unit", sortable:false, align:'center'},
        {name:"companyNameSc", sortable:false, align:'center'},
        {name:"purchasePrice",  sortable:false, align:'center'},
		...
		{name:"retArrAmount", sortable:false, align:'center',},
		{name:"cratioTwo", sortable:false, align:'center',formatter:function(val, opts, rowdata){
            if(val!=null){
		...
		{name:"wamountOne",  sortable:false, align:'center'},

		{name:"cratioOne", sortable:false, align:'center',	formatter:function(val, opts, rowdata){
            if(val!=null){
                return (val*100).toString().substring(0,5);
            }else{
                return "";
		...
		{name:"operate", align: 'center',sortable: false, align:'center',width:350,
			formatter:function(val, opts, rowdata){
                var str = "";
                ...
                str += "<a href='javascript:void(0);' onclick='toOtherList(\""+rowdata.hospitalId+"\",\""+rowdata.hospitalName+"\",\"yimiaoOrderdetail\",\"supComppsDayList\")' class='opIcon' title='配送企业'>配送企业</a>";
	...
	$("#gridlist").jqGrid({
        url: "${ctx }/yimiaoOrderdetail/getSupGoods.html",
		mtype:"post",
		datatype:"json",
		autowidth:true,
		colNames:obj.names,
