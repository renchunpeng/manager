...
<title><spring:message code="配送企业汇总分析"/></title>
<%@include file="/WEB-INF/component/commonCSS.jsp"%>
</head>
<body class="skin-blue sidebar-mini fixed skin-blue-light-frame">
	<section class="content-header">
		<h1><spring:message code="配送企业汇总分析" /></h1>
		<ol class="breadcrumb">
			<li><a href="${ctx}/home.html"><i class="fa fa-home"></i><spring:message code="message.lable.first" /></a></li>
			<li class="active"><a href="#"><spring:message code="配送企业汇总分析" /></a></li>
		</ol>
	</section>
...
var obj={
	"names":['配送企业',
			...
			''],
	"model":[
		{ name: "companyNamePs", width: 150, align: 'center' ,sortable: false},
		...
		{name:"operate", width: 330, align: 'center',sortable: false, align:'center',
			formatter:function(val, opts, rowdata){
				var str = "";
				str += "<a href='javascript:void(0);' onclick='toOtherList(\""+rowdata.companyIdPs+"\",\""+rowdata.companyNamePs+"\",\"stdHospital\")' class='opIcon' title='相关医院'>相关医院</a>";
				str += "<a href='javascript:void(0);' onclick='toOtherList(\""+rowdata.companyIdPs+"\",\""+rowdata.companyNamePs+"\",\"stdArea\")' class='opIcon' title='相关地区'>相关地区</a>";
				str += "<a href='javascript:void(0);' onclick='toOtherList(\""+rowdata.companyIdPs+"\",\""+rowdata.companyNamePs+"\",\"yimiaoProcurecatalog\")' class='opIcon' title='相关疫苗'>相关疫苗</a>";
				str += "<a href='javascript:void(0);' onclick='toOtherList(\""+rowdata.companyIdPs+"\",\""+rowdata.companyNamePs+"\",\"stdCompanySc\")' class='opIcon' title='相关生产企业'>相关生产企业</a>";
				return str;
			}
    	},
    	{name:"companyIdPs", sortable:false, align:'center', hidden:true}
	]
};
...
function initGrid(){
	...
	$("#gridlist").jqGrid({
		url:"${ctx}/yimiaoOrderdetail/getCompanyPsStatistics.html",
		mtype:"post",
		datatype:"json",
		autowidth:true,
		shrinkToFit:false,
...
//跳转页面
function toOtherList(companyIdPs,companyNamePs,type){
	$("#companyIdPs").val(companyIdPs);
	$("#companyNamePs").val(companyNamePs);
...
function queryCount(){
	...
	var queryData = $("#searchForm").serializeJSON();
	$.ajax({
		url:"${ctx}/yimiaoOrderdetail/getYimiaoCompanyPsCount.html",
		data:queryData,
		dataType:'json', 
...
$("#exportExcel").click(function(){
	if(queryInit()==false){
		...
		var queryData = $("#searchForm").serializeJSON();
		$.StandardPost("${ctx}/yimiaoOrderdetail/exportDataCompanyPs.html",queryData);
	}
});
</script>
</html>