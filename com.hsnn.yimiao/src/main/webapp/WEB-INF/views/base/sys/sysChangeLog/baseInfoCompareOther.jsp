...
<div>
	...
	<script src="${ctx}/lib/js/htmlcompare.js"></script>
					...
					var tr="<tr id='comsc' class='form-group-sm'>";
							...
							tr+="<td>";
			    		   	    tr+="<label class='control-label autoTd'>";
			    		   	 	...
			    		   	 	if($(title[0]).get(i)=="公告内容"){
				    		   	 	if($(filed[j+1]).get(i)!= undefined){
				    		   	 		tr+=htmldiff.compare($(filed[j]).get(i),$(filed[j+1]).get(i));
				    		   	 	}else{
				    		   	 		tr+=$(filed[j]).get(i);
				    		   	 	}
			    		   	 	}else if($(title[0]).get(i)=="公告标题" && $(title[0]).get(i)!=""){
			    		   	 		if($(filed[j+1]).get(i)!= undefined){
			    		   	 			tr+=htmldiff.compare($(filed[j]).get(i),$(filed[j+1]).get(i));
			    		   	 		}else{
				    		   	 		tr+=$(filed[j]).get(i);
