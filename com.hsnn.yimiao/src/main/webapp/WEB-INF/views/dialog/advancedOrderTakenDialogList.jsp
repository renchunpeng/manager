...
<body class="skin-blue sidebar-mini fixed skin-blue-light-frame">
	    	...
	    	<div class="box-body">
				<div id="baseInfo" class="box-body">
					<table class="form-table form-table-2 form-table-info">
						...
						<tbody id="tableDatas">
							<tr class="form-group-sm" id="firstTr">
								<td>
									<div class="col-div">
										<select id="column" name="column" class="column" style="width: 125px;">
											<option value="distribute_time">配送时间</option>
											<option value="company_id_ps">配送企业</option>
											<option value="invoice_id">发票号</option>
											...
											<option value="company_id_sc">生产企业</option>
											<option value="source_id">来源名称</option>
										</select>
									</div>
								</td>
		                        <td>
		                        	<select name="direction" id="direction" class="direction" style="width: 125px;">
	...
	<div class="pagetip" style="text-align: left;">
		<h4>注:</h4>
		<div>最多可增加四个查询字段。</div>
		<div>当没有设置时按：配送企业 升序，配送时间 降序，产品Id 升序。</div>
	</div>
</body>
		...
		if(orderInfos) {
			...
			if(arr.length > 0) {
				$.each(arr, function(i, v){
					var arr2 = v.trim().split(" ");
					var order = {};
					order.sortName = arr2[0];
					order.sortDirection = arr2[1];
					aOrder.push(order);
				});
			}
			...
			//依据现有排序规则显示
			if(aOrder.length > 0) {
				$.each(aOrder,function(i,order){
					var appentTr = $("#firstTr").clone();
					if(i == 0) {
						$(".column").find("option[value='"+order.sortName+"']").attr("selected",true);
						$(".direction").find("option[value='"+ order.sortDirection + "']").attr("selected",true);
					} else {
						var trCount = $("#tableDatas").find("tr").length;
					    for (var i = 0; i < trCount; i++) {
			                var selectVal = $("#tableDatas>tr").eq(i).find("select:eq(0) option:selected").val();
			                appentTr.find(".column").find("option[value=" + selectVal + "]").remove();
			            }
						appentTr.find(".column").find("option[value='"+order.sortName+"']").attr("selected",true);
						appentTr.find(".direction").find("option[value='"+ order.sortDirection + "']").attr("selected",true);
						$("#tableDatas").append(appentTr);
					}
		     });
