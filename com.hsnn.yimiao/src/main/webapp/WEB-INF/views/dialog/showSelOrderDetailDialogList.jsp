			...
			<div class="box-body">
				<form action="#" id="searchForm" method="post">
					<div class="input">
						...
						<div class="textalign1">发票号码<spring:message code="message.yanZheng.maoHao"/></div>
	                    <div class="boxinput">
	                        <input type="text" class="textinput"  id="invoiceId" name="invoiceId"  placeholder="发票号码" maxlength="50" />
	                    </div>
...
<script type="text/javascript">
	var obj ={
	        ...
	        "model": [
	        	...
	        	{ name: "payDrugDetail.invoiceId", align: 'center',sortable: false
	        	},
	...
	$(function(){
		...
		$("#gridlist").jqGrid({
            url: "${ctx }/payOrder/getPayDetailChoosed.html",
            colNames: obj.names,
            colModel: obj.model,
            sortable:true,  //是否可排序
			sortname: "a.invoice_id,t.storage_date",// 初始化默认排序字段（多个参数以逗号隔开）
			sortorder: "desc",
            postData: {"payOrderCode":payOrderData.payOrderCode,"storageTag":1},
            multiselect: true,
            caption: "" 
        });
	...
	function closeDialog() {
		 window.top.dialog({id:'orderDetail'}).close();
		 window.top.dialog({id:'orderDetail'}).remove();
	}
	...
	function cancel(type,storageId,realAmount) {
		...
		if(type == 2) {//批量取消
	        ...
	        if (rowIds.length <= 0) {
	        	top.$.HN.message.alert("请选择支付订单明细！", "消息", "warn");
	            return;
	        }
        ...
        $.HN.message.confirm(title, '', '').on(function (e) {
        	  if (e) {
        		  ...
        		  var postData = {"payOrderCode":payOrderData.payOrderCode,"storageIds":str,"strAmounts":strAmounts};
        		   $.post("${ctx }/payOrder/cancel.html",postData, function(result) {
                       if (result.success) {
