...
<script type="text/javascript">
	...
	var orderDetailIds = '${orderDetailIds}';
	orderDetailIds = orderDetailIds.split(",");
	var opt = '${opt}';
    	...
    	$("#gridlist").jqGrid({
            ...
            onSelectRow: function(id) {
            	if(!multiselectFlag) {
    				...
    				var opt = '';
    				if(orderdetailState == declinedToConfirm) {//分为急救和正常订单，急救订单传参  jjyplsdd来区分
    					confirmTip = '<spring:message code="message.refuse.response.ask"/>';
    					successTip = '<spring:message code="message.refuse.response.success"/>';
    					failTip = '<spring:message code="message.refuse.response.fail"/>';
    					opt = "${jjyplsdd}" || <spring:eval expression="T(com.hsnn.medstgmini.util.enums.OptionType).getKeyByItemName('CONFIRM')"></spring:eval>;
    				} else if(orderdetailState == declinedToShip) {
    					confirmTip = '<spring:message code="message.refuse.ship.ask"/>';
    					successTip = '<spring:message code="message.refuse.ship.success"/>';
    					failTip = '<spring:message code="message.refuse.ship.fail"/>';
    					opt = <spring:eval expression="T(com.hsnn.medstgmini.util.enums.OptionType).getKeyByItemName('SHIP')"></spring:eval>;
    				}
    				$.HN.message.confirm(confirmTip, '', '').on(function (e) {
    					if (e && checkData('${orderDetailIds}', opt)) {
    						$.post("${ctx}/drugpurPurchaseOrderdetailRecent/refuseDrugpurPurchaseOrderdetailRecent.html", 
    								{"ids":'${orderDetailIds}', "refuseReason":refuseReason, "orderdetailState":orderdetailState}, function(result) {
    		                    if (result.success) {
    		                 	   closeMyDialog(result.success);
    		                 	   $.alert(successTip, '<spring:message code="message.HN.alert.type"/>', "success");
    		                    } else {
    		                 	   $.alert(result.msg || failTip, "error");
    		                    }
    	                	}, "json");
    					}
    		    	});
	...
	// 确定
	var declinedToConfirm = <spring:eval expression="T(com.hsnn.medstgmini.drug.enums.OrderdetailState).getKeyByItemName('DECLINED_TO_CONFIRM')"></spring:eval>;
	var declinedToShip = <spring:eval expression="T(com.hsnn.medstgmini.drug.enums.OrderdetailState).getKeyByItemName('DECLINED_TO_SHIP')"></spring:eval>;
	var confirmTip = '';
	var successTip = '';
	var failTip = '';
	function confirm() {
		if(!multiselectFlag) {// 选择拒绝理由
			var orderDetailIds = $("#gridlist").find("tr[class*= ui-state-highlight]");
			if(orderDetailIds.length > 0) {
				var refuseReason = $(orderDetailIds[0]).find("td[aria-describedby=gridlist_refuseReason]").attr("title");
				var orderdetailState = '${orderdetailState}';
				var opt = '';
				if(orderdetailState == declinedToConfirm) {
					confirmTip = '<spring:message code="message.refuse.response.ask"/>';
					successTip = '<spring:message code="message.refuse.response.success"/>';
					failTip = '<spring:message code="message.refuse.response.fail"/>';
					opt = <spring:eval expression="T(com.hsnn.medstgmini.util.enums.OptionType).getKeyByItemName('CONFIRM')"></spring:eval>;
				} else if(orderdetailState == declinedToShip) {
					confirmTip = '<spring:message code="message.refuse.ship.ask"/>';
					successTip = '<spring:message code="message.refuse.ship.success"/>';
					failTip = '<spring:message code="message.refuse.ship.fail"/>';
				...
				$.alert("<spring:message code='message.choose.only.one'/>");
			}
		} 
	}
	...
	function checkData(orderDetailIds, optionType) {
		var flag = true;
		$.ajax({
			url:"${ctx}/drugpurPurchaseOrderdetailRecent/optionChecks.html", 
			data:{"ids":'${orderDetailIds}', "optionType":optionType},
			async:false,
			type:"POST",  
			dataType:"json",
